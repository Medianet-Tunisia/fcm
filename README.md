...
Medianet-FCM is an easy to use package working with both Laravel for sending push notification with [Firebase Cloud Messaging](https://firebase.google.com/docs/cloud-messaging/) (FCM).
...
Two types of messages can be sent using Medianet-FCM:
...
> Note: Topic names must be managed by your app and known by your server. The Medianet-FCM package or fcm doesn't provide an easy way to do that.

The following use statement is required for the examples below:

```php
use MedianetFCM\Message\Topics;
```
...
Medianet-FCM supports options based on the options of Firebase Cloud Messaging. These options can help you to define the specificity of your notification.
...
For topics message, Medianet-FCM offers an easy to use api which abstract firebase conditions. To make the condition given for example in the firebase official documentation it must be done with Medianet-FCM like below:
...
```php
$numberSucess = 2;
$mockResponse = new \MedianetFCM\Mocks\MockDownstreamResponse(numberSucess);
...
$sender = Mockery::mock(\MedianetFCM\Sender\FCMSender::class);
